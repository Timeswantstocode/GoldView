...
export default function App() {
  // ---------------------------------------------------------
  // 1. CACHE & STATE INITIALIZATION
  // ---------------------------------------------------------
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_v19_metal') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_v19_forex') || '[]'));
  ...
  useEffect(() => {
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        setPriceData(json);
        localStorage.setItem('gv_v19_metal', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        const transformed = json.data.payload.map(day => ({
          date: day.date,
          // Buyer gets the "BUY" rate from NRB (approx 145.23 currently)
          usdRate: parseFloat(day.rates.find(r => r.currency.iso3 === 'USD')?.buy || 0),
          rates: day.rates
        })).sort((a, b) => new Date(a.date) - new Date(b.date));
        
        setForexHistory(transformed);
        localStorage.setItem('gv_v19_forex', JSON.stringify(transformed));
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: { 
        ...
        tooltip: { 
            enabled: true,
            backgroundColor: 'rgba(25, 25, 25, 0.8)', // GLASS EFFECT
    ...
    scales: {
      x: {
        ...
        ticks: { 
          ...
          maxRotation: 0, 
          // 7D shows 7 dates, 1M/3M shows 8 dates to fill the space
    ...
    onClick: (e, elements) => {
      if (elements.length > 0) {
        ...
        // Instant update for the historical box below
            ...
            {calcMode === 'jewelry' ? (
              ...
              <div className="space-y-6">
                ...
                <div className="p-10 rounded-[3.5rem] text-black text-center shadow-xl transition-all" style={{ background: `linear-gradient(135deg, ${themeColor}, ${activeMetal === 'gold' ? '#b8860b' : '#4b5563'})` }}>
                   ...
                   <h3 className="text-3xl sm:text-5xl font-black tracking-tighter break-all">
