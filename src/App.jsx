...
export default function App() {
  ...
  const [priceData, setPriceData] = useState(() => {
    const saved = localStorage.getItem('cache_metals');
    return saved ? JSON.parse(saved) : [];
  });
  const [forexHistory, setForexHistory] = useState(() => {
    const saved = localStorage.getItem('cache_forex');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    ...
    fetch(`${DATA_URL}?t=${Date.now()}`)
      ...
      .then(json => {
        setPriceData(json);
        localStorage.setItem('cache_metals', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY)
      ...
      .then(json => {
        const transformed = json.data.payload.map(day => ({
          date: day.date,
          // We use the BUY rate because that is what the receiver in Nepal gets
  ...
  // 4. CHART SETUP (RESTORED ORIGINAL GRID & SCALE LOGIC)
  ...
  const chartOptions = useMemo(() => ({
    ...
    scales: {
      x: {
        ...
        ticks: { color: 'rgba(255, 255, 255, 0.3)', font: { size: 10, weight: '700' }, padding: 10, maxTicksLimit: timeframe === 7 ? 7 : 8 }
      },
      y: { 
        display: true, 
        position: 'right', 
      ...
      {view === 'dashboard' ? (
        <main className="px-6 space-y-6 relative z-10 animate-in fade-in duration-700">
          ...
          <section className="bg-white/5 border border-white/10 rounded-[3.5rem] p-9 backdrop-blur-3xl shadow-xl">
            ...
            {/* HISTORICAL DETAIL BOX (RESTORED) */}
            {selectedPoint && (
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-green-700 p-12 rounded-[3.5rem] text-black text-center shadow-xl">
                     ...
                     <h3 className="text-5xl font-black tracking-tighter">
                        {(() => {
                          ...
                          // Using BUY rate for receiver accuracy
