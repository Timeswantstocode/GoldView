...
export default function App() {
  ...
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_v10_metal') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_v10_forex') || '[]'));
  ...
  useEffect(() => {
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        setPriceData(json);
        localStorage.setItem('gv_v10_metal', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        ...
        localStorage.setItem('gv_v10_forex', JSON.stringify(transformed));
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans pb-40 overflow-x-hidden relative">
            ...
            {calcMode === 'jewelry' ? (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <input type="number" style={{ caretColor: themeColor }} className="w-full bg-black/60 border-2 border-zinc-800 p-5 rounded-3xl text-center font-black text-2xl text-white outline-none" onFocus={(e) => e.target.style.borderColor = themeColor} onBlur={(e) => e.target.style.borderColor = '#27272a'} value={calc[u]} onChange={(e) => setCalc({...calc, [u]: e.target.value})} /></div>))}
                </div>
              ...
              /* --- CURRENCY CONVERTER (REFINED FIX) --- */
                  ...
                  <div className="bg-black/40 rounded-[3rem] p-7 border border-white/10 space-y-8">
                    ...
                    <div className="flex items-center justify-between px-2">
                        ...
                        <div className="flex-1 flex flex-col gap-1.5 transition-all duration-300">
                            <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">YOU SEND</p>
                        ...
                        {/* RIGHT COLUMN */}
                        <div className="flex-1 flex flex-col items-end gap-1.5 transition-all duration-300">
                            <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">RECEIVER GETS</p>
      ...
      <nav className="fixed bottom-12 left-10 right-10 h-20 bg-zinc-900/60 backdrop-blur-[50px] rounded-[3rem] border border-white/10 flex justify-around items-center px-4 z-50 shadow-2xl">
        ...
        <button onClick={() => { setView('calculator'); if(activeMetal === 'usd') setActiveMetal('gold'); }} className={`flex flex-col items-center gap-1.5 px-12 py-3.5 rounded-[2.2rem] transition-all duration-300 ${view === 'calculator' ? 'text-black' : 'text-zinc-500'}`} style={view === 'calculator' ? { backgroundColor: themeColor } : {}}>
