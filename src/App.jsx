...
export default function App() {
  ...
  // ---------------------------------------------------------
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_v9_metals') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_v9_forex') || '[]'));
  ...
  useEffect(() => {
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        setPriceData(json);
        localStorage.setItem('gv_v9_metals', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        ...
        localStorage.setItem('gv_v9_forex', JSON.stringify(transformed));
  ...
  // ---------------------------------------------------------
  // 4. CHART CONFIGURATION (ZERO-LATENCY CLICK)
  // ---------------------------------------------------------
  ...
  const chartOptions = useMemo(() => ({
    ...
    onClick: (e, elements) => {
      if (elements.length > 0) {
        ...
        // Instant state update for Historical Box
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans pb-40 overflow-x-hidden relative">
        ...
        <main className="px-6 relative z-10 animate-in zoom-in-95 duration-500">
          <div className="bg-white/5 border border-white/10 rounded-[4rem] p-8 backdrop-blur-3xl shadow-xl">
            ...
            {calcMode === 'jewelry' ? (
              ...
              /* --- CURRENCY MODE (FIXED UI) --- */
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-black/40 rounded-[3rem] p-7 border border-white/10 space-y-8">
                    
                    {/* FIXED GRID: LABELS STAY PUT, CONTENT SWAPS */}
                    <div className="flex items-center justify-between px-2">
                        {/* LEFT COLUMN: ALWAYS "YOU SEND" ROLE */}
                        <div className="flex-1 flex flex-col gap-1.5 transition-all duration-300">
                            <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">YOU SEND</p>
                        ...
                        {/* RIGHT COLUMN: ALWAYS "RECEIVER GETS" ROLE */}
                        <div className="flex-1 flex flex-col items-end gap-1.5 text-right transition-all duration-300">
                            <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">RECEIVER GETS</p>
                            <div className="flex items-center gap-3 justify-end">
                                {currCalc.isSwapped ? (
                                    <>
                                        <select className="bg-transparent font-black text-xl text-white outline-none text-right" value={currCalc.source} onChange={(e) => setCurrCalc({...currCalc, source: e.target.value})}>
                                            {currencyList.map(c => <option key={c.code} value={c.code} className="bg-zinc-900">{c.code}</option>)}
                                        </select>
                                        <span className="text-3xl">{currencyList.find(c => c.code === currCalc.source)?.flag}</span>
                                    </>
      ...
      <nav className="fixed bottom-12 left-10 right-10 h-20 bg-zinc-900/60 backdrop-blur-[50px] rounded-[3rem] border border-white/10 flex justify-around items-center px-4 z-50 shadow-2xl">
        <button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1.5 px-12 py-3.5 rounded-[2.2rem] transition-all duration-300 ${view === 'dashboard' ? 'text-black' : 'text-zinc-500'}`} style={view === 'dashboard' ? { backgroundColor: themeColor, boxShadow: `0 0 40px ${themeColor}40` } : {}}>
