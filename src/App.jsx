...
export default function App() {
  // 1. STATE & CACHE (Instant Boot)
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_metal_final') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_forex_final') || '[]'));
  ...
  const getDayDiff = (id) => {
    ...
    const curr = source[source.length-1][id === 'usd' ? 'usdRate' : id];
    const prev = source[source.length-2][id === 'usd' ? 'usdRate' : id];
  ...
  const chartOptions = useMemo(() => ({
    ...
    scales: {
      ...
      y: { position: 'right', grid: { display: true, color: 'rgba(255, 255, 255, 0.08)', borderDash: [5, 5] }, ticks: { display: false } }
    },
    onClick: (e, elements) => {
  ...
  return (
    ...
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans pb-40 overflow-x-hidden relative">
      <Helmet>
          <title>Gold Price Nepal Today | Live USD to NPR Rate</title>
          <meta name="description" content="Check live 24K Gold, Silver and USD rates in Nepal with official NRB data." />
      </Helmet>
               ...
               return (
                <div key={type} onClick={() => { setActiveMetal(type); setSelectedPoint(null); }}
                  ...
                  <div className="flex justify-between items-start mb-2 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <div className={`px-2.5 py-1 rounded-xl border ${getDayDiff(type).isUp ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>{getDayDiff(type).val}</div>}
                  </div>
            ...
            {selectedPoint && (
              <div className="mt-8 bg-black/80 border-2 rounded-[2.8rem] p-7 flex flex-wrap gap-5 justify-between items-center animate-in slide-in-from-bottom-2 shadow-2xl w-full border-white/10" style={{ borderColor: `${themeColor}80` }}>
                <div className="flex items-center gap-5 flex-1 min-w-[220px]">
                  ...
                  <div><p className="text-[9px] font-black uppercase tracking-[0.3em] mb-1" style={{ color: themeColor }}>Historical Point</p><p className="text-lg font-black text-white">{new Date(selectedPoint.date.replace(' ', 'T')).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p></div>
                </div>
        ...
        <main className="px-6 relative z-10 animate-in zoom-in-95 duration-500">
          <div className="bg-white/5 border border-white/10 rounded-[4rem] p-8 backdrop-blur-3xl shadow-xl">
            <div className="flex p-1 bg-black/40 rounded-3xl mb-10 border border-white/5">
                <button onClick={() => setCalcMode('jewelry')} style={calcMode === 'jewelry' ? { backgroundColor: themeColor } : {}} className={`flex-1 py-4 rounded-2xl text-[10px] font-black uppercase transition-all duration-500 ${calcMode === 'jewelry' ? 'text-black shadow-lg shadow-white/5' : 'text-zinc-500'}`}>Jewelry</button>
            ...
            {calcMode === 'jewelry' ? (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {['tola', 'aana', 'lal'].map((u) => (<div key={u}><label className="text-[10px] font-black text-zinc-500 uppercase mb-2 block ml-3 tracking-[0.2em]">{u}</label>
                  <input type="number" style={{ caretColor: themeColor }} className="w-full bg-black/60 border-2 border-zinc-800 p-5 rounded-3xl text-center font-black text-2xl text-white outline-none focus:border-white/20 transition-all" value={calc[u]} onChange={(e) => setCalc({...calc, [u]: e.target.value})} /></div>))}
                </div>
                    ...
                    <div className="flex items-start justify-between px-1">
                        <div className="flex-1 flex flex-col items-start gap-4">
                            ...
                            <div className="flex flex-col items-center gap-1.5">
                                ...
                                <select className="bg-transparent font-black text-[11px] text-white outline-none mt-1" value={currCalc.source} onChange={(e) => setCurrCalc({...currCalc, source: e.target.value})}>
                                    {currencyList.map(c => <option key={c.code} value={c.code} className="bg-zinc-900">{c.code}</option>)}
                                </select>}
                            </div>
                        </div>
                        <div className="px-4 pt-8">
                            <button onClick={() => setCurrCalc({...currCalc, isSwapped: !currCalc.isSwapped})} className="p-4 bg-green-500/20 rounded-2xl active:rotate-180 transition-all duration-500 border border-green-500/20 shadow-lg shadow-green-500/10"><ArrowRightLeft className="w-5 h-5 text-green-500" /></button>
                        </div>
                        <div className="flex-1 flex flex-col items-end gap-4 text-right">
                            ...
                            <div className="flex flex-col items-center gap-1.5">
                                <span className="text-4xl leading-none">{currCalc.isSwapped ? currencyList.find(c => c.code === currCalc.source)?.flag : '🇳🇵'}</span>
                                {currCalc.isSwapped ? <select className="bg-transparent font-black text-[11px] text-white outline-none mt-1" value={currCalc.source} onChange={(e) => setCurrCalc({...currCalc, source: e.target.value})}>
                                    {currencyList.map(c => <option key={c.code} value={c.code} className="bg-zinc-900">{c.code}</option>)}
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-green-700 p-12 rounded-[3.5rem] text-black text-center shadow-xl relative overflow-hidden group">
                     ...
                     <div className="flex flex-col items-center gap-2 mb-2 relative z-10">
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-black/10 rounded-full border border-black/5">
                            <span className="text-[10px] font-black">{currCalc.isSwapped ? '🇳🇵 NPR' : currencyList.find(c => c.code === currCalc.source)?.flag}</span>
                            <ArrowDown className="w-3 h-3 opacity-40" />
                            <span className="text-[10px] font-black bg-white/20 px-2 rounded-md">{currCalc.isSwapped ? currencyList.find(c => c.code === currCalc.source)?.flag : '🇳🇵 NPR'}</span>
                        </div>
      ...
      <footer className="mt-12 px-8 pb-12 text-zinc-600 text-[10px] leading-relaxed border-t border-white/5 pt-10">
        ...
        <p>GoldView provides real-time updates for 24K Chhapawal Gold and Pure Silver rates in Nepal.</p>
        <h2 className="text-zinc-400 font-black mt-6 mb-2 uppercase tracking-widest">NRB Official Forex Rates</h2>
        <p>Stay updated with USD to NPR, GBP to NPR and other currencies using official NRB data.</p>
      </footer>
