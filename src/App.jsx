...
export default function App() {
  ...
  // ---------------------------------------------------------
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_prod_metal') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_prod_forex') || '[]'));
  ...
  useEffect(() => {
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        setPriceData(json);
        localStorage.setItem('gv_prod_metal', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        ...
        localStorage.setItem('gv_prod_forex', JSON.stringify(transformed));
  ...
  // ---------------------------------------------------------
  // 3. DYNAMIC THEME ENGINE (HEADER & UI SYNC)
  // ---------------------------------------------------------
  ...
  const chartOptions = useMemo(() => ({
    ...
    onClick: (e, elements) => {
      if (elements.length > 0) {
        ...
        // Ensuring state updates instantly for the detail box
               ...
               return (
                <div key={type} onClick={() => { setActiveMetal(type); setSelectedPoint(null); }}
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {type === 'usd' && forexLoading ? <div className="animate-pulse"><RefreshCcw className="w-3 h-3 text-green-500 animate-spin" /></div> : 
            ...
            {selectedPoint && (
              <div className="mt-8 bg-black/80 border-2 rounded-[2.8rem] p-7 flex flex-wrap gap-5 justify-between items-center animate-in slide-in-from-bottom-2 shadow-2xl w-full border-white/10" style={{ borderColor: `${themeColor}80` }}>
                <div className="flex items-center gap-5 flex-1 min-w-[220px]">
                  ...
                  <p className="text-lg font-black text-white">{new Date(selectedPoint.date.replace(' ', 'T')).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p></div>
                </div>
            ...
            {calcMode === 'jewelry' ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <input type="number" style={{ caretColor: themeColor }} className="w-full bg-black/60 border-2 border-zinc-800 p-5 rounded-3xl text-center font-black text-2xl text-white outline-none focus:border-white/20" value={calc[u]} onChange={(e) => setCalc({...calc, [u]: e.target.value})} /></div>))}
                </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-black/40 rounded-[3rem] p-7 border border-white/10 space-y-8">
                    ...
                    <div className="flex items-center justify-between px-1">
                        ...
                        {/* Always on the right: Role Receive */}
                        <div className="flex-1 flex flex-col items-end gap-2 text-right">
                            ...
                            <div className="flex items-center gap-3 justify-end">
                                {currCalc.isSwapped ? (
                                    ...
                                    <><span className="text-lg font-black">NPR</span><span className="text-3xl">🇳🇵</span></>
                                )}
                            </div>
                        </div>
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-green-700 p-12 rounded-[3.5rem] text-black text-center shadow-2xl relative overflow-hidden group">
                     ...
                     <p className="text-[8px] font-bold mt-4 opacity-50 uppercase tracking-widest relative z-10">Live NRB Official Buying Rate</p>
                  </div>
              </div>
            )}
          </div>
