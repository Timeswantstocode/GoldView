...
import { 
  ...
  X, Calendar, Zap, Activity, Coins, ArrowRightLeft, Globe, ArrowDown, History, Share2, BellRing, BellOff, Info
} from 'lucide-react';
...
// --- ORIGINAL FOGGY TOOLTIP HANDLER (PRESERVED) ---
...
export default function App() {
  // --- STATES ---
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_v27_metal') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_v27_forex') || '[]'));
  ...
  const [calcType, setCalcType] = useState('buy'); // Default: New Purchase
  ...
  useEffect(() => {
    ...
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        ...
        localStorage.setItem('gv_v27_metal', JSON.stringify(processed));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        if (json?.data?.payload) {
            ...
            localStorage.setItem('gv_v27_forex', JSON.stringify(transformed));
        }
    });
  }, []);
            ...
            <section className="bg-white/5 border border-white/10 rounded-[3.5rem] p-10 backdrop-blur-3xl shadow-xl">
              ...
              <div className="h-64 relative w-full">
                <Line 
                  ...
                  scales: { x: { grid: { display: false }, ticks: { color: 'rgba(255,255,255,0.2)', font: { size: 10, weight: '900' }, maxRotation: 0 } }, y: { display: false } },
                  onClick: (e, elements) => { if (elements.length > 0) { const idx = elements[0].index; const p = filteredData[idx]; setSelectedPoint({ index: idx, date: p.date, price: activeMetal === 'usd' ? 0 : p[activeMetal] }); } } }} 
                />
              </div>

              {selectedPoint && (
                <div className="mt-10 bg-white/[0.08] backdrop-blur-3xl border border-white/10 rounded-[2.8rem] p-7 flex justify-between items-center w-full animate-in slide-in-from-bottom duration-500">
                  <div className="flex items-center gap-5">
                    ...
                    <div><p className="text-[10px] font-black uppercase tracking-widest opacity-50">Log Point</p>
                    <p className="text-xl font-black">{new Date(selectedPoint.date.replace(' ', 'T')).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p></div>
                  </div>
                  <div className="flex items-center gap-8">
                    <div className="text-right"><p className="text-[10px] font-black uppercase opacity-50">Price</p>
                    <p className="text-3xl font-black">{activeMetal === 'usd' ? 'Point' : formatRS(selectedPoint.price)}</p></div>
          ...
          <main className="px-6 relative z-10 mt-6 animate-in zoom-in-95 duration-500">
            <div className="bg-white/5 border border-white/10 rounded-[4rem] p-10 backdrop-blur-3xl shadow-xl">
              <div className="flex p-2 bg-black/40 backdrop-blur-xl rounded-[2.2rem] mb-12 border border-white/10">
                  ...
                  <button onClick={() => setCalcMode('currency')} className={`flex-1 py-4 rounded-[1.8rem] text-[10px] font-black uppercase transition-all ${calcMode === 'currency' ? 'bg-green-500 text-black shadow-xl' : 'text-zinc-500'}`}>Currency</button>
              </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex gap-2 justify-center">
                      {['gold', 'tejabi', 'silver'].map(m => (
                        <button key={m} onClick={() => setActiveMetal(m)} className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase border backdrop-blur-xl transition-all ${activeMetal === m ? 'border-white/40 bg-white/10 text-white shadow-xl' : 'border-white/5 opacity-30'}`}>
                    ...
                    <div className="space-y-6">
                      ...
                      <div onClick={() => setCalc({...calc, vat: !calc.vat})} className="p-7 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] flex justify-between items-center cursor-pointer transition-all active:scale-95">
                        <div className="flex items-center gap-4">
                          <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${calc.vat ? 'bg-amber-500 shadow-[0_0_15px_rgba(212,175,55,0.4)]' : 'bg-zinc-800'}`}><Zap className={`w-4 h-4 ${calc.vat ? 'text-black fill-black' : 'text-zinc-500'}`} /></div>
                  ...
                  <div className="p-14 rounded-[4.5rem] text-center shadow-2xl transition-all duration-1000 relative" style={{ background: `linear-gradient(145deg, ${themeColor}, #000 160%)`, color: 'white' }}>
                     <p className="text-[12px] font-black uppercase tracking-[0.4em] mb-3 opacity-60">{calcType === 'buy' ? 'Jewelry Price' : 'You Receive'}</p>
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-black/60 backdrop-blur-2xl rounded-[3.5rem] p-8 border border-white/10 space-y-10">
                        <div className="flex items-start justify-between">
                            <div className="flex-1"><p className="text-[10px] font-black text-zinc-600 mb-4 uppercase">SEND</p>
                            ...
                            <div className="flex-1 text-right"><p className="text-[10px] font-black text-zinc-600 mb-4 uppercase">GET</p>
