...
export default function App() {
  ...
  // SAFETY CHECK: If data.json is empty or hasn't loaded, show a loading screen
  if (!priceData || priceData.length === 0) {
    return (
      <div className="min-h-screen bg-black text-yellow-500 flex items-center justify-center p-10 text-center">
        <div>
          <h1 className="text-2xl font-bold animate-pulse">GOLDVIEW</h1>
          <p className="text-gray-500 mt-2">Initializing market data...</p>
          <p className="text-xs mt-4 text-gray-700">If this persists, run the Scraper Action in GitHub.</p>
        </div>
      </div>
    );
  }
  ...
  const calculateTotal = () => {
    const totalTola = parseFloat(calc.tola || 0) + (parseFloat(calc.aana || 0) / 16) + (parseFloat(calc.lal || 0) / 192);
    ...
    <div className="min-h-screen bg-black text-gray-100 p-4 font-sans">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <div 
          ...
          className={`p-4 rounded-2xl border-2 transition-all cursor-pointer ${activeTab === 'gold' ? 'border-yellow-500 bg-yellow-950/20' : 'border-gray-800'}`}
        >
        ...
        <div 
          ...
          className={`p-4 rounded-2xl border-2 transition-all cursor-pointer ${activeTab === 'silver' ? 'border-gray-400 bg-gray-900/40' : 'border-gray-800'}`}
        >
      ...
      <div className="bg-gradient-to-b from-gray-900 to-black p-6 rounded-3xl border border-yellow-900/30">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <input type="number" placeholder="Tola" className="bg-gray-800 p-3 rounded-xl text-white" onChange={e => setCalc({...calc, tola: e.target.value})} />
          <input type="number" placeholder="Aana" className="bg-gray-800 p-3 rounded-xl text-white" onChange={e => setCalc({...calc, aana: e.target.value})} />
          <input type="number" placeholder="Lal" className="bg-gray-800 p-3 rounded-xl text-white" onChange={e => setCalc({...calc, lal: e.target.value})} />
        </div>
        <input type="number" placeholder="Making Charges (Rs)" className="w-full bg-gray-800 p-3 rounded-xl mb-4 text-white" onChange={e => setCalc({...calc, making: e.target.value})} />
