...
export default function App() {
  // 1. STATE & PERSISTENT CACHE (Instant Boot Logic)
  const [priceData, setPriceData] = useState(() => JSON.parse(localStorage.getItem('gv_final_metal') || '[]'));
  const [forexHistory, setForexHistory] = useState(() => JSON.parse(localStorage.getItem('gv_final_forex') || '[]'));
  ...
  useEffect(() => {
    ...
    fetch(`${DATA_URL}?t=${Date.now()}`).then(res => res.json()).then(json => {
        setPriceData(json);
        localStorage.setItem('gv_final_metal', JSON.stringify(json));
        setLoading(false);
    ...
    fetch(FOREX_PROXY).then(res => res.json()).then(json => {
        const transformed = json.data.payload.map(day => ({
          date: day.date,
          // Buying Rate: Amount receiver gets in Nepal (e.g. 145.23)
          usdRate: parseFloat(day.rates.find(r => r.currency.iso3 === 'USD')?.buy || 0),
          rates: day.rates
        })).sort((a, b) => new Date(a.date) - new Date(b.date));
        
        setForexHistory(transformed);
        localStorage.setItem('gv_final_forex', JSON.stringify(transformed));
  ...
  // 3. THEME ENGINE (Synced across Tabs)
  ...
  // 4. CHART CONFIG (GRID, LABELS, GLASS TOOLTIP)
  ...
  return (
    ...
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans pb-40 overflow-x-hidden relative">
      <Helmet>
          <title>Gold Price Nepal Today | Live USD to NPR Rate - GoldView</title>
          <meta name="description" content="Check live Gold, Silver and official NRB exchange rates in Nepal with integrated calculators." />
      </Helmet>
          ...
          <div className="bg-white/5 border border-white/10 rounded-[4rem] p-8 backdrop-blur-3xl shadow-xl">
            ...
            {calcMode === 'jewelry' ? (
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="bg-black/40 rounded-[3rem] p-7 border border-white/10 space-y-10">
                    <div className="flex items-start justify-between relative px-2">
                        ...
                        <div className="absolute left-1/2 -translate-x-1/2 top-6">
                            <button onClick={() => setCurrCalc({...currCalc, isSwapped: !currCalc.isSwapped})} className="p-4 bg-green-500/20 rounded-2xl active:rotate-180 transition-all duration-300 border border-green-500/20 shadow-lg shadow-green-500/10"><ArrowRightLeft className="w-5 h-5 text-green-500" /></button>
                        </div>
                        {/* ROLE RECEIVER GETS */}
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-green-700 p-10 rounded-[3.5rem] text-black text-center shadow-xl relative overflow-hidden group">
                     ...
                     <p className="text-[8px] font-bold mt-4 opacity-40 uppercase tracking-widest relative z-10">NRB Buying Rate Applied</p>
                  </div>
              </div>
            )}
          </div>
      ...
      <footer className="mt-12 px-8 pb-32 text-zinc-600 text-[10px] leading-relaxed border-t border-white/5 pt-10 text-center">
        ...
        <p className="max-w-xs mx-auto">Real-time 24K Gold, Silver and official NRB Forex exchange rates for Nepal.</p>
        <a href="https://github.com/Timeswantstocode" target="_blank" rel="noopener noreferrer" className="mt-10 flex flex-col items-center gap-2 transition-all duration-300 hover:opacity-100 opacity-30 group">
